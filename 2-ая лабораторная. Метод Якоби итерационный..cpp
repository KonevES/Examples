...
void method_yakobi(double** matrix, double* answer_vector, double* x_vector_now, double* x_vector_next, int n){
    int i,j,k;
    bool flag;
    do {
        for (i=0; i<n; i++){
            x_vector_next[i]=answer_vector[i];
            for (j=0; j<n; j++){
                if (i!=j){
                    x_vector_next[i]-=(matrix[i][j]*x_vector_now[j]);
                }
            }
            x_vector_next[i]/=matrix[i][i];
        }
...
int main(){
    ...
    method_yakobi(matrix,answer_matrix,x_vector_now,x_vector_next,n);
    for (i=0; i<n; i++){
        std::cout<<x_vector_next[i]<<" ";
    }
    std::cout<<std::endl;
